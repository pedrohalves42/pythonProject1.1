...
def valida(cpnj):
    cnpj = apenas_numeros(cpnj)
    try:
        if sequenciador(cnpj):
            return False
    except:
        return False
    try:
...
def gera():
    ...
    inicio_cnpj = f'{primeiro_digito}{segundo_digito}{segundo_bloco}{terceiro_bloco}{mil_contra}00'
...
def formata(cnpj):
    ...
    formatado = f'{cnpj[:3]}.{cnpj[2:5]}.{cnpj[5:8]}/{cnpj[8:12]}-{cnpj[12:14]}'
